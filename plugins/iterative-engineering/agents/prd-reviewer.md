...
When reviewing a brainstorm (not a formal PRD), adjust calibration. Brainstorms are earlier in the pipeline — they explore approaches and make recommendations. The most valuable review feedback for a brainstorm expands the author's thinking: unconsidered alternatives, missing evaluation criteria, risks of the chosen approach, buy-vs-build considerations, assumptions that deserve validation. Structural issues (vague next steps, thin problem statements) are still valid findings but should generally be MEDIUM, not HIGH — a brainstorm is expected to be less rigorous than a PRD. Reserve HIGH for findings that would lead to choosing the wrong approach if unaddressed.
...
- **Exhaustive completeness** — not every possible gap is a finding. Only flag missing information when its absence creates a concrete risk: a tech planner forced to guess, a dependency that could derail the timeline, or an assumption that stakeholders might disagree on. "No retention policy mentioned" is a finding if the system stores unbounded data; it's not a finding if retention is an obvious ops concern with no product implications.
